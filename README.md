<div align="center">
  <img src="https://via.placeholder.com/150x150/20232a/61dafb?text=Aegis+L7" width="120" height="120" alt="Logo">
  <h1>🛡️ Aegis-L7 Defense Plugin</h1>
  <p><strong>완벽한 메모리 관리와 Thread-Safe 설계를 갖춘 마인크래프트 차세대 L7 봇넷/매크로 방어 플러그인</strong></p>
...
## 플러그인은 개발 단계에 있습니다. 미확인된 오류가 있기때문에 확인 후 픽스 중입니다. 개발진이 발견하지 못한 버그는 이슈로 넘겨주세요.
</div>
...
2. [프록시 서버 주의사항 (중요)](#-프록시bungeecord--velocity-서버-사용-시-주의사항)
3. [핵심 기능 및 전체 모듈 설명](#-핵심-기능-및-전체-모듈-설명)
4. [자주 묻는 질문 / 트러블슈팅](#-자주-묻는-질문-faq--트러블슈팅)
5. [명령어 및 권한](#-명령어-및-권한)
...
2. GitHub **[Releases]** 탭에서 가장 최신 버전의 Aegis-L7 (Latest Build).jar 파일을 다운로드합니다.
3. 서버의 plugins/ 폴더에 .jar 파일을 넣고 서버를 구동합니다.
4. plugins/L7DefensePlugin/config.yml 파일이 자동 생성되면 설정을 확인합니다.
5. (선택) Redis 동기화를 사용하려면 config.yml의 endgame-features.redis-sync 섹션에 호스트 정보를 입력하고 /l7def reload를 입력하세요.
...
**해결 방법 (config.yml 수정):**
\\\yaml
# 아래 3가지 모듈은 반드시 false로 설정하세요! (v1.0.0 부터 기본값 false 적용 완료)
ping-before-join:
...
* **서버 리소스팩 검증:** 인게임에 들어오기 전 가벼운 투명 리소스팩을 수락(Accept)해야만 통과시킵니다.
...
* **Ping Flood 역공격 최적화 (SLP Crasher):** 핑 폭발 공격을 하는 봇에게 기형적인 OOM 유발 페이로드를 반사합니다.

### 🔗 5. 중앙 동기화 및 관리 (Sync & Admin)
* **Redis Pub/Sub 클러스터링:** 차단된 악성 IP를 Redis 채널을 통해 연결된 모든 BungeeCord 및 하위 서버에 0.1초 만에 실시간으로 전파합니다.
* **실시간 Admin GUI:** 인게임에서 /l7def gui를 통해 모듈의 상태를 확인하고 원클릭으로 켜고 끌 수 있습니다.
...
A. 내부 블랙리스트(메모리 캐시)에 등록되었기 때문입니다. 서버를 재부팅하거나 일정 시간(기본 10분)이 지나면 차단이 해제됩니다. (OP 또는 화이트리스트 유저는 예외 처리되어 있습니다.)
